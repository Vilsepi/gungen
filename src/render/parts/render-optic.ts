...
export function renderOptic(part: LayoutPart): string {
  return `
    <g class="part optic" transform="translate(${part.x} ${part.y}) rotate(${part.rotationDeg})">
      <rect x="${-part.length / 2}" y="${-part.width / 2}" width="${part.length}" height="${part.width}" rx="${part.width * 0.28}" />
      <rect x="${-part.length * 0.18}" y="${part.width * 0.3}" width="${part.length * 0.36}" height="${part.width * 0.24}" rx="2" opacity="0.75" />
