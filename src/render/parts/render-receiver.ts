...
export function renderReceiver(part: LayoutPart, prng: Prng): string {
  ...
  for (let index = 0; index < cuts; index += 1) {
    ...
    inner += `<rect x="${x}" y="${-part.width * 0.12}" width="${part.length * 0.07}" height="${part.width * 0.24}" rx="2" opacity="0.55" />`;
  }
  ...
  return renderRoundedBody(part, "part receiver", inner, 0.15);
}
